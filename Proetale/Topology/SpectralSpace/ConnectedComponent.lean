...
instance t2Space_connectedComponent {X : Type u} [TopologicalSpace X]  [CompactSpace X]
  ...
  have hU' : IsClopen (ConnectedComponents.mk '' U : Set (ConnectedComponents X)) :=
    IsClopen.connectedComponents_image_isClopen hU
  -- Use clopen set to separate: mk '' U and (mk '' U)ᶜ are disjoint open neighborhoods
  refine ⟨ConnectedComponents.mk '' U, (ConnectedComponents.mk '' U)ᶜ, hU'.2,
    hU'.1.isOpen_compl, Set.mem_image_of_mem _ hxU, ?_, disjoint_compl_right⟩
