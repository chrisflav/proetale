...
This complements `CategoryTheory.Abelian.Ext.comp_assoc_of_second_deg_zero` and
`CategoryTheory.Abelian.Ext.comp_assoc_of_third_deg_zero`. -/
@[simp]
lemma comp_assoc_of_first_deg_zero {X Y Z T : D} {a₂ a₃ a₂₃ : ℕ}
    (α : Ext X Y 0) (β : Ext Y Z a₂) (γ : Ext Z T a₃) (h₂₃ : a₂ + a₃ = a₂₃) :
    (α.comp β (zero_add _)).comp γ h₂₃ = α.comp (β.comp γ h₂₃) (zero_add _) := by
  apply comp_assoc
  lia
...
theorem Sheaf.exists_mem_ext_map_eq_zero (F : Sheaf J AddCommGrpCat.{u + 1}) (p : ℕ) {U : C}
  ...
  | zero =>
    ...
    rw [← Ext.comp_assoc_of_first_deg_zero, Ext.mk₀_comp_mk₀, key, ← Ext.mk₀_comp_mk₀,
  ...
  | succ p ih =>
    ...
    refine ⟨R, hR, ?_⟩
    intro W g hg
    rw [← Ext.comp_assoc_of_first_deg_zero, H g hg, Ext.zero_comp]
