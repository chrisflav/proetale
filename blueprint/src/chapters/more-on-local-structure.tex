...
\begin{proof}
    ...
    \leanok
    Let $I \subseteq B$ be a finitely generated ideal. Since $B$ is weakly étale, $B$ is
    $A$-flat, so its $A$-submodule $I$ is $A$-flat by \Cref{lemma:weak-dim-flat-submodule}.
    By \Cref{lemma:weakly-etale-flat} it follows that $I$ is $B$-flat. Hence $B$ is of weak
    dimension $\le 1$.
\end{proof}

\begin{lemma}
    Let $(A_i)_{i \in I}$ be a family of rings and for every $i$ an $A_i$-module $M_i$.
    Let $N$ be a $\prod_{i \in I} A_i$-submodule of $\prod_{i \in I} M_i$ and let
    $N_i \subseteq M_i$ be the image of $N$ under the $i$-th projection.
    Then $N \subseteq \prod_{i \in I} N_i$.
    If $N$ is finitely generated, equality holds.
    \label{lemma:prod-submodule-fg}
    \lean{Submodule.FG.mem_pi}
    \leanok
\end{lemma}

\begin{proof}
    \leanok
    The inclusion $N \subseteq \prod_{i \in I} N_i$ is clear. Assume $N$ is finitely generated
    and pick a finite generating set $s$. Let $y \in \prod_{i \in I} M_i$ with $y_i \in N_i$ for
    every $i$. For each $i$ choose $z^{(i)} \in N$ with $z^{(i)}_i = y_i$ and write
    $z^{(i)} = \sum_{a \in s} c^{(i)}_a \cdot a$ with $c^{(i)}_a \in \prod_{j} A_j$. Setting
    $b_a := (i \mapsto (c^{(i)}_a)_i) \in \prod_j A_j$, we have $y = \sum_{a \in s} b_a \cdot a$,
    since componentwise $y_i = z^{(i)}_i = \sum_{a \in s} (c^{(i)}_a)_i \cdot a_i$. Hence $y \in N$.
\end{proof}
...
\begin{lemma}[\stacksproject{092U}]
    ...
    \lean{IsIntegrallyClosedIn.exists_weakDimensionLEOne_isEpi_exact}
    \leanok
\end{lemma}
...
\begin{proof}
    ...
    \leanok
    For every $x \in L \setminus A$, the element $x$ is not integral over $A$, since $A$ is
    integrally closed in $L$. Hence there exists a valuation subring $A \subseteq V_x \subseteq L$
    with $x \notin V_x$ (\stacksproject{090P}). Set
    \[
        S = \prod_{x \in L \setminus A} V_x \quad \text{and} \quad T = \prod_{x \in L \setminus A} L.
    \]
    Then $S$ is of weak dimension $\le 1$ by
    \Cref{lemma:prod-valuation-ring-weak-dim-integrally-closed}. Since each $V_x$ has fraction
    field $L$, the map $S \to T$ is the localization at the non-zerodivisors of $S$ (which are
    exactly the componentwise non-vanishing elements), hence flat, injective and an epimorphism.
    The square is cartesian: if $l \in L$ and $s \in S$ have the same image in $T$, then
    $l \in V_x$ for all $x \in L \setminus A$, so $l \in A$, since otherwise $l \notin V_l$.
\end{proof}
...
\begin{lemma}[\stacksproject{092W}]
    ...
    \lean{Algebra.WeaklyEtale.isIntegrallyClosedIn_tensorProduct}
    \leanok
\end{lemma}
...
\begin{proof}
    ...
    \leanok
    Choose a cartesian diagram as in \Cref{lemma:integrally-closed-cartesian-weak-dim} and
    tensor it with the flat $A$-algebra $B$. The resulting diagram with vertices $B$,
    $B \otimes_A L$, $B \otimes_A S$ and $B \otimes_A T$ is again cartesian. The ring
    $B \otimes_A S$ is weakly étale over $S$, hence of weak dimension $\le 1$ by
    \Cref{lemma:weakly-etale-preserve-weak-dim}, and $B \otimes_A S \to B \otimes_A T$ is a
    flat, injective ring epimorphism, since all three properties are stable under (flat) base
    change. By \Cref{lemma:weak-dim-integrally-closed}, $B \otimes_A S$ is integrally closed
    in $B \otimes_A T$. An element of $B \otimes_A L$ integral over $B$ maps to an element of
    $B \otimes_A T$ integral over $B \otimes_A S$, hence lying in $B \otimes_A S$; by
    cartesianness it lies in $B$.
\end{proof}
...
\begin{proof}
    \uses{lemma:integral-local-algebraic-residue-field, lemma:tensorprod-local-purely-inseparable,
    lemma:henselian-local-integral-local, lemma:weakly-etale-integrally-closed}
    \leanok
...
\begin{proof}
    \leanok
    \uses{thm:modify-pi0-profinite-properties, thm:identifies-local-ring-to-top-fully-faithful,
    thm:isom-of-identifies-local-rings-w-local-isom-pi0}
    Let $T = \pi_0(\Spec B)$ with its continuous map to $\pi_0(\Spec A)$. By
    \Cref{thm:modify-pi0-profinite-properties}, there is an ind-Zariski $A$-algebra $A'$
    with $\Spec A' = \Spec A \times_{\pi_0(\Spec A)} T$. The universal property of the
    pullback gives a continuous map $\Spec B \to \Spec A'$ over $\Spec A$, which by
    fully faithfulness (\Cref{thm:identifies-local-ring-to-top-fully-faithful}) comes
    from an $A$-algebra map $A' \to B$. This map is a w-local map of w-local rings that
    identifies local rings and induces a bijection on connected components, hence is an
    isomorphism by \Cref{thm:isom-of-identifies-local-rings-w-local-isom-pi0}. Thus
    $B \cong A'$ is ind-Zariski over $A$.
\end{proof}
...
\begin{proof}
    \leanok
    \uses{thm:weakly-etale-over-sh, lemma:w-local-bijective-on-stalks-ind-zariski,
    thm:ind-etale-w-strictly-local-cover}
    Let $C$ be a faithfully flat, ind-étale, w-strictly local $R$-algebra
    (\Cref{thm:ind-etale-w-strictly-local-cover}) and let $J \subseteq C$ be the ideal
    cutting out the closed points of $\spec{C}$. Set $D' = C \otimes_R S$, which is
    weakly étale over $C$ and faithfully flat ind-étale over $S$, and let $D$ be the
    w-localization of $D'$ along $J D'$ (\stacksproject{097A}). Then $D$ is w-local,
    ind-Zariski over $D'$, and the closed points of $\spec{D}$ lie over closed points of
    $\spec{C}$. Since the local rings of $C$ at closed points are strictly henselian,
    the weakly étale map $C \to D$ identifies local rings by
    \Cref{thm:weakly-etale-over-sh}, hence is ind-Zariski by
    \Cref{lemma:w-local-bijective-on-stalks-ind-zariski}. Thus $T = D$ works:
    $S \to D' \to D$ is faithfully flat and ind-étale (faithful flatness of the
    composition uses that the closed points of $\spec{C}$ surject onto $\spec{R}$), and
    $R \to C \to D$ is ind-étale.
\end{proof}
