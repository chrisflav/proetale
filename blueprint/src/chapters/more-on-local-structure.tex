...
\begin{lemma}
    ...
    \label{lemma:ind-etale-weakly-etale-algebra}
    \lean{Algebra.IndEtale.weaklyEtale, RingHom.IndEtale.weaklyEtale}
    \leanok
    \uses{def:ind-etale, def:weakly-etale-algebra, lemma:etale-weakly-etale-algebra}
\end{lemma}

\begin{proof}
    \leanok
    Write the ind-étale $R$-algebra $S$ as a filtered colimit $S = \colim_{i} S_{i}$ of étale
    $R$-algebras. Each $S_{i}$ is weakly étale by \Cref{lemma:etale-weakly-etale-algebra}, so each
    $S_{i}$ is $R$-flat and each multiplication map $S_{i} \otimes_{R} S_{i} \to S_{i}$ is flat.
    Flatness of $S$ over $R$ follows since flat modules are closed under filtered colimits. For the
    multiplication map, $S \otimes_{R} S \to S$ is the filtered colimit of the maps
