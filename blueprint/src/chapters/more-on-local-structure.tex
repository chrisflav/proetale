...
\begin{lemma}[{\cite[\href{https://stacks.math.columbia.edu/tag/092Y}{Tag 092Y}]{stacks-project}}]
    ...
    $R \to S$ is integral and $\kappa(S) / \kappa(R)$ or $\kappa(T) / \kappa(R)$
...
\begin{proof}
    \leanok
    Write $A = T \otimes_{R} S$. Integrality is stable under base change, so
    $T \to A$ and $S \to A$ are integral. Hence by going-up every maximal ideal
    $\mathfrak{q}$ of $A$ contracts to $\mathfrak{m}_T$ along $T \to A$ and to
    $\mathfrak{m}_S$ along $S \to A$, so $\mathfrak{q}$ contains the ideal
    $J = \mathfrak{m}_T A + \mathfrak{m}_S A$. In particular $J$ is contained in the
    Jacobson radical of $A$, so it suffices to show that the quotient $A / J$ is local.
    Using the base change of quotients twice, this quotient is identified with
    \[
        A / J \cong \kappa(T) \otimes_{R} \kappa(S) \cong \kappa(T) \otimes_{\kappa(R)} \kappa(S)
    .\] By assumption one of $\kappa(S) / \kappa(R)$, $\kappa(T) / \kappa(R)$ is purely
    inseparable; using the symmetry $\kappa(T) \otimes_{R} \kappa(S) \cong
    \kappa(S) \otimes_{R} \kappa(T)$ we reduce both cases to the same statement. A purely
    inseparable extension induces a universal homeomorphism on spectra, so
    $\spec{\kappa(T) \otimes_{\kappa(R)} \kappa(S)}$ is a single point and the tensor
    product of the residue fields is local.
\end{proof}
