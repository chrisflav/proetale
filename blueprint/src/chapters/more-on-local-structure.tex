...
\begin{lemma}[\stacksproject{00U3}]
    Let $F : J \to \mathrm{CommRing}$ be a filtered diagram of commutative rings with
    colimit cocone $c$, and let $\varphi : c.\mathrm{pt} \to A$ be a finitely presented
    ring map. Then there exists a stage $j_0 \in J$, a commutative ring $A_{j_0}$,
    a finitely presented ring map $\varphi_{j_0} : F(j_0) \to A_{j_0}$ and a ring map
    $\psi : A_{j_0} \to A$ such that the square
    \[
    \begin{tikzcd}
        F(j_0) \arrow{r}{c_{j_0}} \arrow{d}{\varphi_{j_0}} & c.\mathrm{pt} \arrow{d}{\varphi} \\
        A_{j_0} \arrow{r}{\psi} & A
    \end{tikzcd}
    \] is a pushout.
    \label{lemma:fp-algebra-descent-filtered-colim}
    \lean{CommRingCat.exists_finitePresentation_isPushout_of_isColimit}
    \leanok
\end{lemma}

\begin{proof}
    \leanok
    Choose a finite presentation $A = c.\mathrm{pt}[x_1, \ldots, x_n] / (f_1, \ldots, f_m)$.
    The polynomials $f_1, \ldots, f_m$ involve only finitely many coefficients in
    $c.\mathrm{pt}$. Since $J$ is filtered and $c$ is a filtered colimit cocone, the underlying
    functor to types is also a filtered colimit, hence there exists a stage $j_0 \in J$ such
    that all these coefficients lift to $F(j_0)$. Define $A_{j_0}$ by the same presentation
    over $F(j_0)$. Then $A \cong c.\mathrm{pt} \otimes_{F(j_0)} A_{j_0}$, which exhibits the
    desired pushout square.
\end{proof}
