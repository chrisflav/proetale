...
\begin{lemma}[{\cite[\href{https://stacks.math.columbia.edu/tag/08ZL}{Tag 08ZL}]{stacks-project}}]
...
\begin{lemma}
  ...
  \uses{thm:pi0-to-totally-disconnected}
...
\begin{proof}
  ...
  If \(z \nin \overline{\{(x, y)\}}\), there exists opens \(U \subset X\), \(V \subset Y\) such that \(z \nin U \times V\), thus \((x, y) \nin U \times V\). Either \(x \nin U\) or \(y \nin V\), suppose \(x \nin U\). Then \(U \cap \overline{\{x\}} = \emptyset\), thus \(U \times V \cap \overline{\{x\}} \times \overline{\{y\}}  = \emptyset\). This shows that \(\overline{\{x\}} \times \overline{\{y\}} \subseteq \overline{\{(x, y)\}}\).
\end{proof}
...
\begin{proof}
  \uses{thm:pi0-profinite,thm:spectral-product,thm:closed-subspace-spectral,thm:pi0-to-totally-disconnected,thm:pi0-to-totally-disconnected-injective}
  Note that $Y$ is a closed subspace of $X \times T$ (see \verb`TopCat.pullbackIsoProdSubtype`) as $\pi_0(X)$ is a profinite space (\Cref{thm:pi0-profinite}) hence Hausdorff. Since $X \times T$ is spectral (\Cref{thm:spectral-product}) it follows that $Y$ is spectral (\Cref{thm:closed-subspace-spectral}). Let $Y \to \pi_0(Y) \to T$ be the canonical factorization in \Cref{thm:pi0-to-totally-disconnected}. It is clear that $\pi_0(Y) \to T$ is surjective. The fibres of $Y \to T$ are homeomorphic to the fibres of $X \to \pi_0(X)$. Hence these fibres are connected. It follows that $\pi_0(Y) \to T$ is injective by \Cref{thm:pi0-to-totally-disconnected-injective}. We conclude that $\pi_0(Y) \to T$ is a homeomorphism since it is a bijective map from a quasi-compact space to a Hausdorff space by \verb`isHomeomorph_iff_continuous_bijective`.
\end{proof}
...
\begin{proof}
  \uses{thm:cartesian-profinite,thm:closed-points-isom-pi0}
...
\begin{lemma}
  ...
  \uses{def:w-local-ring}
  Let $A$ be a w-local ring. Then $\Spec (A)$ is the disjoint union of the spectra of the local rings at closed points.
\end{lemma}
...
\begin{proof}
  \uses{thm:closed-points-isom-pi0,thm:w-local-set-decomposition,thm:localization-isom-of-going-down}
  Let $X^c \subset X = \Spec (A)$ and $Y^c \subset Y = \Spec (B)$ be the sets of closed points. By assumption $Y^c$ maps into $X^c$ and the induced map $Y^c \to X^c$ is a bijection since \Cref{thm:closed-points-isom-pi0} holds. By \Cref{thm:w-local-set-decomposition}, we see that both $X$ and $Y$ , as sets, are disjoint unions of the spectra of the local rings at closed points. As \(A \to B\) identifies local rings, $Y \to X$ is a bijection.

  Since $A \to B$ is flat (can be checked on stalks of \(B\)), it has going down (see \verb`Algebra.HasGoingDown.of_flat`). Thus \Cref{thm:localization-isom-of-going-down} shows for any prime $\q \subset B$ lying over $\p \subset A$ we have $B_{\q} = B_{\p B}$. Since $B_{\q} = A_{\p}$ by assumption, we see that $A_{\p} = B_{\p B}$ for all primes $\p$ of $A$. Thus $A = B$ since isomorphism can be checked locally on $A$.
\end{proof}
...
\begin{proof}
  \uses{def:colim-open-closed-localizations,thm:colim-open-closed-localizations-surj-ind-zariski,thm:finite-product-identifies-local-rings,thm:ind-Zariski-identifies-local-rings,thm:identifies-local-rings-composition,thm:identifies-local-ring-to-top-fully-faithful,thm:cartesian-profinite}
  Write $T = \lim T_i$ as the limit of an inverse system of finite discrete spaces over a directed set. For each $i$ let $Z_i = \operatorname{Im} (T \to \pi_0(X) \times T_i)$. This is a closed subset. Observe that $X \times T_i$ is the spectrum of $A_i = \prod_{t \in T_i} A$ and that $A \to A_i$ is a local isomorphism. By \Cref{def:colim-open-closed-localizations,thm:colim-open-closed-localizations-surj-ind-zariski}, we see that $Z_i \subset \pi_0(X \times T_i) = \pi_0(X) \times T_i$ corresponds to a surjection $A_i \to B_i$ which is ind-Zariski such that $\Spec (B_i) = (X \times T_i) \times_{\pi_0(X) \times T_i} Z_i = X \times_{\pi_0(X)} Z_i$ as subsets of $X \times T_i$. The transition maps $T_i \to T_{i'}$ induce maps $Z_i \to Z_{i'}$ and $X \times_{\pi_0(X)} Z_i \to X \times_{\pi_0(X)} Z_{i'}$. Since the both the ring maps of \(A \to A_{i} \to B_{i}\) identifies local rings by \Cref{thm:finite-product-identifies-local-rings,thm:ind-Zariski-identifies-local-rings}, their composition also identifies local ring by \Cref{thm:identifies-local-rings-composition}. Now apply \Cref{thm:identifies-local-ring-to-top-fully-faithful}, we
...
\begin{proof}
  ...
  \uses{thm:extremally-disconnected-cover,thm:exists-ind-Zariski-cartesian,thm:cartesian-profinite-w-local,thm:ind-Zariski-identifies-local-rings,thm:w-contractible-if}
