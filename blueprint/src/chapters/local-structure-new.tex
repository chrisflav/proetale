...
The following pure topological lemma will be used in \Cref{thm:colim-open-closed-localizations-surj-ind-zariski}

\begin{lemma}[{\cite[\href{https://stacks.math.columbia.edu/tag/04PL}{Tag 04PL} (b)\(\imples\)(a)]{stacks-project}}]
...
\begin{proof}
  It is clear that (a) implies (b). Assume (b). Let $x \in X$, $x \notin T$. Let $x \in C \subset X$ be the connected component of $X$ containing $x$. TBA. % By Lemma 005F we see that $C = \bigcap V_\alpha$ is the intersection of all open and closed subsets $V_\alpha$ of $X$ which contain $C$. In particular, any pairwise intersection $V_\alpha \cap V_\beta$ occurs as a $V_\alpha$. As $T$ is a union of connected components of $X$ we see that $C \cap T = \emptyset$. Hence $T \cap \bigcap V_\alpha = \emptyset$. Since $T$ is quasi-compact as a closed subset of a quasi-compact space (see Lemma 005C) we deduce that $T \cap V_\alpha = \emptyset$ for some $\alpha$, see Lemma 005D. For this $\alpha$ we see that $U_\alpha = X \setminus V_\alpha$ is an open and closed subset of $X$ which contains $T$ and not $x$. The lemma follows.
\end{proof}
...
\begin{lemma}[{\cite[\href{https://stacks.math.columbia.edu/tag/096V}{Tag 096V}]{stacks-project}}]
    Let $Z \subseteq \spec{A}$ be a locally closed subscheme of the form
...
\begin{definition}
    Let $Z \subseteq \spec{A}$ be a locally closed subset of the form $D(f) \cap V(I)$.
    We denote the $A$-algebra $S^{-1}A$ constructed in \ref{lemma:locally-closed-specializations} by
    $A_{\tilde{Z}}$.
...
\begin{lemma}
    Let $Z \subseteq \spec{A}$ be a locally closed subset of the form $D(f) \cap V(I)$. Then
    $A_{\tilde{Z}}$ is ind-Zariski over $A$.
    \uses{def:tilde-locclosed}
    \label{lemma:tilde-locclosed-ind-zariski}
\end{lemma}

\begin{proof}
    $A_{\tilde{Z}} = S^{-1} A$ for some multiplicative subset $A$.
\end{proof}
...
\begin{definition}
    Let $A$ be a ring and $E \subseteq A$ a subset. Let
    \[
...
\begin{proof}
    ...
    as a finite product of ind-Zariski algebras. Thus $A_w$ is
    a filtered colimit of ind-Zariski algebras, hence ind-Zariski.
\end{proof}
...
\begin{lemma}
    The closed points of $\spec{A_w}$ surject onto $\spec{A}$.
    \uses{def:w-localization}
...
\begin{proposition}
    The ring $A_w$ is $w$-local.
    \uses{def:w-localization}
    \label{prop:w-localization-w-local}
\end{proposition}
...
\begin{proof}
    By \ref{prop:w-localization-w-local}, the ring $A_w$ is w-local, so the set of closed points
    ...
    Since the closed points of $\spec{A_w}$ surject onto $\spec{A}$ and $B' / IB' \cong B / I B$, it
...
\begin{proof}
    ...
    The last statement follows from \Cref{thm:ind-Zariski-is-flat-ind-etale} and the fact that if a flat ring map induces a surjection on closed points (guaranteed by \label{item:w-local-closed-points-isom-w-local}), then it is faithfully flat.
\end{proof}
...
\begin{proof}
  ...
  Choose a continuous section to the surjective continuous map $V(IB) \to V(I)$. This is possible as $V(I) = \Spec (A)^c \cong \pi_0(\Spec (A))$ (by \Cref{thm:closed-points-isom-pi0}) is extremally disconnected and use \Cref{thm:extremally-disconnected-projective}. The image is a closed subspace $T \subset \pi_0(\Spec (B)) \cong V(IB)$ (as it is the continuous image of \(V(B)\), a closed subset of a compact topological space) mapping homeomorphically onto $\pi_0(\Spec (A))$. Since the ring map \(B \to B_T\) constructed in 
  \Cref{def:colim-open-closed-localizations} is surjective, ind-Zariski and inducing homeomorphism $\pi_0(\Spec (B_T)) \cong \pi_0(\Spec (A))$ by
  \Cref{thm:colim-open-closed-localizations-surj-ind-zariski}.By \Cref{thm:surjective-w-local-ring}, \(B_T\) is w-local as a quotient of \(B\) and the map \(B \to B_T\) is also w-local. We know that \(B \to B_T\) identifies local rings by \Cref{thm:ind-Zariski-identifies-local-rings}. If we post compose the ring map \(A \to B\) with \(B \to B_T\), then it is still w-local by \Cref{thm:w-local-map-composition} and identifies local rings by \Cref{thm:identifies-local-rings-composition}. At this point $A \to B$ is an isomorphism by \Cref{thm:isom-of-identifies-local-rings-w-local-isom-pi0}.
\end{proof}
