...
\begin{lemma}[{\cite[\href{https://stacks.math.columbia.edu/tag/0A31}{Tag 0A31}, (3) $\implies$ (4)]{stacks-project}}]
  ...
  \uses{def:generalization}
  \lean{generalizationHull.eq_sUnion_of_isCompact}
  \leanok
\end{lemma}
...
\begin{proof}
    \uses{thm:colim-ideal-lim-zero-locus,lemma:finite-stratification-ideal-bijection,lemma:closure-limit-intersection,thm:closure-limit-compatible}
...
\begin{lemma}
    ...
    \lean{Ideal.WLocalization.zeroLocus_map_algebraMap_eq_closedPoints,Ideal.WLocalization.quotientMap_algebraMap_bijective}
    \leanok
    ...
    \begin{enumerate}
        ...
        \item the map $A / I \to A_{w, I} / I A_{w, I}$ is an isomorphism.
    \end{enumerate}
\end{lemma}
...
\lean{Ideal.WLocalization.quotientMap_algebraMap_bijective,Ideal.WLocalization.algebraMap_specComap_preimage_closedPoints_eq}
\leanok
...
\begin{proof}
    \uses{lemma:w-localization-closed-points, lemma:closed-closed-points-tilde-w-local, lemma:tilde-locclosed-ind-zariski}
...
\begin{corollary}
  Notation as in \Cref{thm:closed-points-isom-w-local}. If further $A \to B$ is faithfully flat, then the composition $A \to B \to C$ is also faithfully flat.
...
\begin{proof}
  \uses{thm:ind-Zariski-is-flat-ind-etale,thm:w-localization-ideal-ind-Zariski,lemma:going-down-surjective-of-closed,thm:closed-points-isom-w-local}
  By \Cref{thm:ind-Zariski-is-flat-ind-etale,thm:w-localization-ideal-ind-Zariski}, $A \to B \to C$ is flat, so by \ref{lemma:going-down-surjective-of-closed} it suffices
  to show every closed point of $A$ is in the image. But since $\spec{B} \to \spec{A}$ is surjective,
  also $V(IC) \cong V(IB) \to V(I)$ is surjective (the first map is an isomorphism by \Cref{item:quotinet-isom-closed-points-isom-w-local} in \Cref{thm:closed-points-isom-w-local}).
\end{proof}
...
\begin{lemma}
    \label{thm:ind-etale-has-retraction-if-ind-etale-plus-c-has}
    \uses{def:w-local-ring,def:ind-etale}
    \lean{IsWLocalRing.exists_retraction_of_exists_retraction_of_zeroLocus_map_eq}
    \leanok
...
\begin{proof}
  \uses{def:w-localization-ideal,cor:closed-points-isom-w-local-faithfully-flat,thm:w-localization-ideal-ind-Zariski,thm:ind-Zariski-is-flat-ind-etale,thm:ind-etale-comp,thm:closed-points-isom-w-local}
  ...
  We may replace $B$ by the ring $C = B_{w, IB}$ (see \Cref{def:w-localization-ideal}). To justify this, note that the map \(A \to C\) is faithfully flat by \Cref{cor:closed-points-isom-w-local-faithfully-flat} and \(B \to C\) is ind-Zariski by \Cref{thm:w-localization-ideal-ind-Zariski}, thus ind-étale by \Cref{thm:ind-Zariski-is-flat-ind-etale}. Therefore, by \Cref{thm:ind-etale-comp}, the composition \( A \to C \) is also ind-étale. Hence, we may assume $\Spec (B)$ is w-local such that the set of closed points of $\Spec (B)$ is $V(IB)$ by \Cref{item:w-local-closed-points-isom-w-local,item:inverse-image-closed-points-isom-w-local} in \Cref{thm:closed-points-isom-w-local}.
\end{proof}
...
\begin{proof}
  \uses{thm:ind-etale-has-retraction-if-ind-etale-plus-c-has,thm:ind-etale-plus-c-has-retraction-if-w-contractible}
  This is \Cref{thm:ind-etale-plus-c-has-retraction-if-w-contractible} with \Cref{thm:ind-etale-has-retraction-if-ind-etale-plus-c-has}.
