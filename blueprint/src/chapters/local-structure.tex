...
\begin{definition}[{\cite[\href{https://stacks.math.columbia.edu/tag/08ZL}{Tag 08ZL}]{stacks-project}}]
  ...
  \lean{ConnectedComponents.lift,ConnectedComponents.lift_comp,ConnectedComponents.lift_comp_apply}
  \leanok
...
\begin{lemma}
  ...
  \lean{ConnectedComponents.injective_lift}
  \leanok
...
\begin{lemma}[{\cite[\href{https://stacks.math.columbia.edu/tag/0906}{Tag 0906}]{stacks-project}}]
  ...
  \lean{totallyDisconnectedSpace_connectedComponent,compactSpace_connectedComponent,t2Space_connectedComponent}
  \leanok
...
\begin{lemma}
  ...
  \lean{Topology.IsClosedEmbedding.quasiSeparatedSpace,Topology.IsClosedEmbedding.spectral,SpectralSpace.of_isClosed}
  \leanok
...
\begin{lemma}
  \label{thm:pullback-Hausdorff}
  \lean{T2Space.pullback,CategoryTheory.IsPullback.t2Space}
  \leanok
  Let \(X\) and \(Y\) be Hausdorff spaces. Let \(f : X \to S\) and \(g : Y \to S\) be continuous maps. Then the fiber product \(X \times_S Y\) is Hausdorff.
\end{lemma}

\begin{proof}
  \leanok
  Note that \(X \times_S Y\) is a subspace of $X \times Y$. (See \verb`TopCat.pullbackIsoProdSubtype`.) Since $X \times Y$ is Hausdorff (\verb`Prod.t2Space`) it follows that \(X \times_S Y\) is Hausdorff (\verb`Topology.IsEmbedding.t2Space`).
\end{proof}
...
\begin{lemma}
  ...
  \leanok
  Let \(X\), \(Y\), and \(S\) be topological spaces. Let \(f : X \to S\) and \(g : Y \to S\) be continuous maps. The natural map \(X \times_S Y \to X \to Y\) is a closed embedding.
\end{lemma}
...
\begin{lemma}
  ...
  \lean{Homeomorph.spectral,Homeomorph.t0Space,Homeomorph.t2Space,Homeomorph.quasiSober,Homeomorph.quasiSeparatedSpace,Homeomorph.quasiCompactSpace,Homeomorph.prespectral}
  \leanok
  Let \(X\) and \(Y\) be topological spaces. If \(X\) is spectral (resp. T0, Hausdorff, quasi-Sober, quasi-separated, quasi-compact, prespectral) and there is a homeomorphism \(X \cong Y\), then \(Y\) is spectral (resp. T0, Hausdorff, quasi-Sober, quasi-separated, quasi-compact, prespectral).
\end{lemma}
...
\begin{lemma}
  ...
  \lean{T0Space.pullback,PrespectralSpace.pullback,SpectralSpace.pullback,QuasiSeparatedSpace.pullback,QuasiSober.pullback,QuasiCompactSpace.pullback,CategoryTheory.IsPullback.t0Space,CategoryTheory.IsPullback.quasiSober,CategoryTheory.IsPullback.quasiSeparatedSpace,CategoryTheory.IsPullback.compactSpace,CategoryTheory.IsPullback.prespectralSpace,CategoryTheory.IsPullback.spectralSpace}
  \leanok
  Let \(X\) and \(Y\) be spectral spaces. Let \(f : X \to S\) and \(g : Y \to S\) be continuous maps. Then the fiber product \(X \times_S Y\) is spectral.
\end{lemma}
...
\uses{def:pi0-to-totally-disconnected}
\lean{ConnectedComponents.lift_bijective_of_isPullback,ConnectedComponents.isHomeomorph_lift_of_isPullback}
...
\end{tikzcd}
\]
be a cartesian diagram in the category of topological spaces with $T$ profinite. Then the canonical map $\pi_0(Y) \to T$ defined in \Cref{def:pi0-to-totally-disconnected} is an isomorphism.
\end{lemma}

\begin{proof}
  \uses{thm:pullback-Hausdorff,thm:pullback-spectral,thm:pi0-profinite,def:pi0-to-totally-disconnected,thm:pi0-to-totally-disconnected-injective}
  By \Cref{thm:pullback-Hausdorff,thm:pullback-spectral}, \(Y\) is spectral and Hausdorff. By \Cref{thm:pi0-profinite}, $\pi_0(X)$ is a profinite space. Let $Y \to \pi_0(Y) \to T$ be the canonical factorization in \Cref{def:pi0-to-totally-disconnected}. It is clear that $\pi_0(Y) \to T$ is surjective. The fibres of $Y \to T$ are homeomorphic to the fibres of $X \to \pi_0(X)$. Hence these fibres are connected. It follows that $\pi_0(Y) \to T$ is injective by \Cref{thm:pi0-to-totally-disconnected-injective}. We conclude that $\pi_0(Y) \to T$ is a homeomorphism since it is a bijective map from a quasi-compact space to a Hausdorff space by \verb`isHomeomorph_iff_continuous_bijective`.
\end{proof}
