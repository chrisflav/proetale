...
:::lemma_ "lemma:ind-etale-weakly-etale" (parent := "local-structure") (uses := "def:ind-etale") (lean := "Algebra.IndEtale.weaklyEtale")
An ind-étale ring map is weakly étale.
(Stacks Project, [Tag 097K](https://stacks.math.columbia.edu/tag/097K))
:::

:::proof "lemma:ind-etale-weakly-etale"
Flatness passes to filtered colimits. For flatness of the multiplication map
$`B \otimes_A B \to B` it suffices, by the purity criterion, that its kernel is
generated by idempotents: every element of the kernel is defined at some étale
stage $`B_i`, where the kernel of $`B_i \otimes_A B_i \to B_i` is generated by
a single idempotent.
:::
