...
:::proof "lem:bij-on-spectrum-of-unique-lying-over-and-residue-field-eq"
Write $`\mu : B \otimes_A B \to B` for the multiplication map and
$`\iota_1, \iota_2 : B \to B \otimes_A B` for the two inclusions. Since
$`\mu \circ \iota_1 = \mathrm{id}_B`, the map $`\iota_1` is a section of $`\mu`,
so $`\mathrm{Spec}(\iota_1)` is a left inverse of $`\mathrm{Spec}(\mu)` and
hence $`\mathrm{Spec}(\mu)` is injective.

For surjectivity, let $`P` be a prime of $`B \otimes_A B` and set
$`\mathfrak{q} = \iota_1^{-1}(P)`. The two pullbacks $`\iota_1^{-1}(P)` and
$`\iota_2^{-1}(P)` have the same image in $`\mathrm{Spec}\, A` (as $`\iota_1`
and $`\iota_2` agree on $`A`), and since $`\mathrm{Spec}\, B \to
\mathrm{Spec}\, A` is injective they coincide, so also $`\mathfrak{q} =
\iota_2^{-1}(P)`. Let $`\mathfrak{p}` be the image of $`\mathfrak{q}` in $`A`.
The two induced maps $`\kappa(\mathfrak{q}) \to \kappa(P)` induced by $`\iota_1`
and $`\iota_2` both restrict to the same map on $`\kappa(\mathfrak{p})`, and
$`\kappa(\mathfrak{p}) \to \kappa(\mathfrak{q})` is surjective, so they agree.
Consequently $`\iota_1` and $`\iota_2` have the same image in $`\kappa(P)`, and
an induction shows that every $`x \in B \otimes_A B` has the same image in
$`\kappa(P)` as $`\iota_1(\mu(x))`. Therefore $`x \in P` if and only if
$`\iota_1(\mu(x)) \in P`, which exhibits $`P` as the pullback of
$`\mathfrak{q}` along $`\mu`.
:::
...
:::proof "lem:isom-of-unique-lying-over-and-residue-field-eq" (uses := "lem:bij-on-spectrum-of-unique-lying-over-and-residue-field-eq, def:weakly-etale-algebra, lem:iso-of-bij-on-spectrum-and-surj-and-flat")
...
{bpref "lem:iso-of-bij-on-spectrum-and-surj-and-flat"}[]. In particular the
inclusion $`\iota_1 : B \to B \otimes_A B` is bijective, being a section of the
bijection $`\mu`. Since $`A \to B` is faithfully flat (it is flat and
surjective on spectra), bijectivity of
$`\iota_1 = \mathrm{id}_B \otimes (A \to B)` descends to surjectivity of
$`A \to B`. As $`A \to B` is flat, surjective and bijective on spectra, it is
an isomorphism by {bpref "lem:iso-of-bij-on-spectrum-and-surj-and-flat"}[].
:::
